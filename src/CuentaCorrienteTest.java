...
public class CuentaCorrienteTest {
	...
	public void testCuentaSeCreaConSaldoCero() {
		...
		assertEquals(BigDecimal.ZERO.setScale(2, RoundingMode.HALF_UP), cuenta.getSaldo()); // Verificamos que el saldo sea cero.
	}
	...
	public void testIngresoNegativoNoCambiaSaldo() {
		...
		assertEquals(0, cuenta.getSaldo().compareTo(BigDecimal.ZERO.setScale(2, RoundingMode.HALF_UP)));
		// Verificamos que el saldo sigue siendo cero.
	}
	...
	public void testIngresoMayorQue6000NoSeRealiza() {
		...
		assertEquals(0, cuenta.getSaldo().compareTo(BigDecimal.ZERO));
		// Verificamos que el saldo sigue siendo cero.
	}
	...
	public void testTransferenciaNegativaNoSeRealiza() {
		...
		// Comparamos con BigDecimal.ZERO.setScale(2, RoundingMode.HALF_UP) para asegurar la misma escala
		assertEquals(BigDecimal.ZERO.setScale(2, RoundingMode.HALF_UP), cuentaDestino.getSaldo()); // Verificamos que la cuenta destino sigue siendo cero.
		assertEquals(new BigDecimal("500.00").setScale(2, RoundingMode.HALF_UP), cuentaOrigen.getSaldo()); // Verificamos que la cuenta origen no cambia.
	}
